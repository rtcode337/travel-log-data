...
```csv
route,series,seq,spot_key,description
サイコロ1,サイコロ1,1,道後温泉,サイコロの出目だけで行き先を決める旅の第1弾。
サイコロ1,サイコロ1,2,松山駅,サイコロの出目だけで行き先を決める旅の第1弾。
```
...
| `description` | | ルートの説明文。travel-log側の地図でルートの線をタップすると出るルート詳細に表示される。ルート単位の値なので、同じ`route`の行にはすべて同じ値を書く(空欄なら説明なし) |
...
- 差分更新: 既存と同名のルートはシリーズ・説明・経由地を丸ごと置き換え、CSVに無い
  ルートには触らない。シリーズ・説明・経由地の並びが既存と完全一致するルートは
  スキップされるため、同じCSVを何度アップロードしてもよい
