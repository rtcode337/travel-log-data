...
- **24×24の箱**にSVGのパスで描く。`icon`があるカテゴリのスポットは、
...
観光地(tourist)では**種類の分かる5つ**(神社仏閣=鳥居・城=天守と石垣・温泉=湯気と湯船・
美術館博物館=神殿・商業施設=買い物袋)を`icon`で描き、ピンの輪郭は丸に戻してある
(**輪郭と中身の両方で種類を示すと二重になる**ため)。**「じっくり」はその後ろ**に置いている —— 種類が分かるほうを
優先し、どれにも当たらないスポット(自然・街並み・その他)でだけ「立ち寄るのに手間が
かかるか」(次項)を形で示す。**この5つのカテゴリからは`じっくり`を外してある**
(2026-08-10。形が1つしか出せない以上、付けたままだと「付いているのに何も示さない」
データが2,300件残るため)。`じっくり`は3,581件→1,281件(自然508・その他755・街並み18)。
