...
**ピンの中にカテゴリのアイコンを描ける**(`icon`)。

```json
{ "category": "温泉", "icon": "M2,13 H22 V15 q0,6 -10,6 q-10,0 -10,-6 Z" }
```

- **24×24の箱**にSVGのパスで描く(`iconViewSize`で一辺を変えられる。配布されている
  アイコンのSVGは`viewBox`が24・48・1000などまちまちなので、**パスを書き換えずに
  そのまま貼れる**ようにするため)。`icon`があるカテゴリのスポットは、
  **シリーズの文字(A〜E)の代わりに**この絵がピンの中に出る
  —— シリーズは色で分かるので、中身は「何の場所か」に使うほうが地図が読める
...
- **ラベル(シリーズの文字)は頭の中心(50,50)に描かれる**ので、**そこは塗りで覆う**
...
観光地(tourist)では**種類の分かる6つ**(神社仏閣=鳥居・城=天守(48×48)・
温泉=湯気と湯船・美術館博物館=神殿・商業施設=買い物袋・自然=針葉樹)を`icon`で描き、ピンの輪郭は丸に戻してある
(**輪郭と中身の両方で種類を示すと二重になる**ため)。**「じっくり」はその後ろ**に置いている —— 種類が分かるほうを
優先し、どれにも当たらないスポット(自然・街並み・その他)でだけ「立ち寄るのに手間が
かかるか」(次項)を形で示す。**これらのカテゴリからは`じっくり`を外してある**
(2026-08-10。形が1つしか出せない以上、付けたままだと「付いているのに何も示さない」
データが残るため)。`じっくり`は3,581件→773件(その他755・街並み18)。

かつては5つとも**輪郭の形**(五重塔・城壁・湯船・神殿・ひさしのある店)で表していたが、
小さいピンでは輪郭が潰れるうえ中心に文字が乗る制約もあったため、
**中に絵を描く方式へ移した**(輪郭は丸)。**形を増やすときは`lib/categoryStyle.ts`の`PIN_SHAPES`・
