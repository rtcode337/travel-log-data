...
観光地(tourist)では**神社・寺院・城・温泉・自然の5つをシリーズにして、地図記号に
ならったアイコン**(鳥居・卍・縄張・湯壺と湯煙・針葉樹)を付けてある。移す前は
カテゴリ側の設定だったが、**カテゴリは複数持てて1つに絞れない**ため、1スポットに
1つと決まるシリーズへ移した(元のカテゴリからは外してある)。
**美術館博物館・商業施設・動物園水族館のアイコンは行き先が無くなった**
(これらは互いに排他とは限らず、シリーズにすると神社・寺院と同じ軸に並んでしまう)。
旧定義はgitの履歴の`category_styles`に残っている。
