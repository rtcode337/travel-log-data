...
スポットを巡った順に矢印で繋ぐルートの定義(列: `route,series,seq,spot_key`)。取り込むと
...
(`route`はルートの表示名で、シリーズとは独立)。`spot_key`は
...
- `tourist/spots.csv`(11,090件): Wikipedia(ja)の**「○○の観光地」47都道府県ページ**を唯一の
...
- `suiyou_dodesho_domestic/spots.csv`: HTB(北海道テレビ)「水曜どうでしょう」のレギュラー放送時
  ...
  **具体的なスポットの採否・説明文・各企画との対応関係は未検証(要確認)。** 2つのパートに
  ...
  連番サフィックス)で全266件に付与。内訳の経緯:

  - 36件は後から追加したもの。うち21件はルート作り直し(後述)の際に、jawikiの行程節に
    地名があるのに未収録だったスポット(サイコロ1・2の出発地の六本木プリンスホテル、
    サイコロ3後編の津名港・神戸ヘリポート・但馬空港ほか)。残り15件は、**企画そのものが
...
- `suiyou_dodesho_domestic/routes.csv`(31ルート): jawiki記事の企画ごとの
  ...
  **行程節に地名が書かれていない立ち寄り先(宿・対決場所など)までは網羅していない。**
  ...
  多摩川緑地、5周年記念…の出発地「大通バスセンター」、クイズ!試験に出るどうでしょうと
  原付東日本のHTB(いずれも座標をchiezo・Nominatimで特定できなかったため見送り。
  HTBは企画当時の南平岸旧社屋で、現社屋の座標とは別)。また企画そのものが未収録のものとして
...
- `suiyou_dodesho_overseas/spots.csv`: 上記`suiyou_dodesho_domestic`の海外編。jawiki記事
  ...
  (「外部データソースを扱う際の注意」参照)。`series`列・`categories`列とも企画名が入り、
