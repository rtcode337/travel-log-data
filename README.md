...
|---|---|---|---|---|
| [tourist](tourist/README.md)<br>(観光地) | 日本全国の観光地。travel-log側の唯一の既定種別 | 10,640 | 説明文: Wikipedia(ja)の記事<br>座標: Wikipedia(ja)の記事・Wikidata | 説明文: CC BY-SA 4.0<br>座標: CC BY-SA 4.0・CC0 |
...
| [restaurant](restaurant/README.md)<br>(有名飲食店) | Wikipedia(ja)に記事がある店に絞った飲食店(網羅ではない。記事に名指しされた店を少数含む) | 236 | 説明文: Wikipedia(ja)の記事<br>座標: OpenStreetMap(1件のみWikipedia) | 説明文: CC BY-SA 4.0<br>座標: ODbL |
...
| [gotouchi_gourmet](gotouchi_gourmet/README.md)<br>(ご当地グルメ) | ご当地グルメ・郷土料理を、**その料理を食べに行く土地**(市区町村・都道府県どまりの行はその地域の主要駅)に置いたもの | 2,391 | 説明文: Wikipedia(ja)の記事(全行の根拠を`evidence.csv`に保持)<br>座標: OpenStreetMap(109件はWikipedia(ja)の記事の座標、1件はOverture Places) | 説明文: CC BY-SA 4.0<br>座標: ODbL(記事由来の109件は CC BY-SA 4.0、1件は CDLA Permissive 2.0) |
...
一部の座標: Overture Maps Foundation (CDLA Permissive 2.0)
