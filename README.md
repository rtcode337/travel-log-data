...
tourist/
  spots.csv
  settings.json
  tourist_excluded_candidates/   # 生成時に除外した候補と理由(都道府県別、参考資料)
post_office/
  spots.csv
  settings.json
goshuin/
...
- `tourist/spots.csv`(11,628件): Wikipedia(ja)の**「○○の観光地」47都道府県ページ**を唯一の
  ...
  客観指標としてパーセンタイル区分している(観光地11,628件中の相対順位)。最上位をSにすると
  ...
  2. リダイレクトを解決して同一記事を1件に統合し、赤リンク・曖昧さ回避・一覧ページ・
     祭事/イベント・市区町村そのものの記事・都道府県そのものの記事を除外
...
- `buratamori/spots.csv`: NHK「ブラタモリ」レギュラー版全5シリーズ(第1シリーズ2009年〜
  ...
  `label`(合体回は先頭の番号のみ、例: `17-18`→`17`)に分離してある。`categories`列は
  `第<N>シリーズ`(1〜5)でシリーズを表す。`settings.json`の`series`は全340話(5シリーズ合計)を
...
- `buratamori/routes.csv`(56ルート): 上記spots.csvの放送回(`series`)ごとの行順を
...
- `suiyou_dodesho_domestic/spots.csv`: HTB(北海道テレビ)「水曜どうでしょう」のレギュラー放送時
  ...
  代用座標を使っている行がある)。`series`列・`categories`列とも企画名
  ...
  **具体的なスポットの採否・説明文・各企画との対応関係は未検証(要確認)。** 2つのパートに
  ...
  連番サフィックス)で全232件に付与
- `suiyou_dodesho_domestic/routes.csv`(27ルート): 上記spots.csvの企画(`series`)ごとの
