...
- `buratamori/spots.csv`: NHK「ブラタモリ」レギュラー版全5シリーズ(第1シリーズ2009年〜
  ...
  放送回の番号(`9`のような数字。合体回は`17-18`、スペシャルは`SP1`のような表記)、
  `category`列は`第<N>シリーズ`(1〜5)でシリーズを表す。放送回のタイトルは各行の
  `description`冒頭に`【#9 品川】`の形式で持つ。`settings.json`の`ranks`は全放送回番号を
  数値順に列挙した機械生成(見た目は統一で、ピンのラベルに番号を表示する)。
  **具体的なスポット名・説明文・各回との対応関係は未検証(要確認)。** シリーズごとに別々の
  ...
  2回目以降の登場回の情報(`rank`の放送回番号・`description`冒頭の放送回タイトル)は
  取り込まれない点に留意
